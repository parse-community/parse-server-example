...
Parse.Cloud.job("LeagueReset", function (request, status) {

	//// CLEAR FIXTURES ////

	var LeagueStr = request.params.League_ID;
	...
	queryFixtures.find({
		success : function (resultsFixtures) {
			Parse.Object.destroyAll(resultsFixtures, {
				success : function () {

					//// BUILD USER ARRAY ////

					var queryLeagueUsers = new Parse.Query("UserLeagueData");
					queryLeagueUsers.equalTo("League", league);

					queryLeagueUsers.find({
						success : function (resultsLeagueUsers) {
							for (var i = 0; i < resultsLeagueUsers.length; i++) {
								resultsLeagueUsers[i].set("Points", 0);
								resultsLeagueUsers[i].set("Wins", 0);
								resultsLeagueUsers[i].set("Loses", 0);
								resultsLeagueUsers[i].set("Games", 0);
								resultsLeagueUsers[i].set("TotalMatchDurationSeconds", 0);
							}
							Parse.Object.saveAll(resultsLeagueUsers);
							var arr = [];
							for (var day = 1; day <= resultsLeagueUsers.length - 1; day++) {
								for (var iUser = 0; iUser < resultsLeagueUsers.length; iUser++) {

									var Fixtures = Parse.Object.extend("Fixtures");
									var fixture = new Fixtures();

									fixture.set("Player0", resultsLeagueUsers[iUser].get("Player"));
									fixture.set("Player1", resultsLeagueUsers[(iUser + day >= resultsLeagueUsers.length) ? iUser + day - resultsLeagueUsers.length : iUser + day].get("Player"));
									fixture.set("League", league);
									fixture.set("GameDay", day);
									var today = new Date();
									var tomorrow = new Date(today);
							...
							Parse.Object.saveAll(arr, {
								success : function (list) {
									...
									var queryLeagues = new Parse.Query("Leagues");
									queryLeagues.equalTo("objectId", LeagueStr);
									queryLeagues.select("Name", "Level", "CurrentDay", "MaxDays");

									queryLeagues.find({
										success : function (resultsLeagues) {
											resultsLeagues[0].set("CurrentDay", 1);
											resultsLeagues[0].save({
												success : function (list) {
													status.success("Okinii");
												},
												error : function (error) {}
											});
										},
...
Parse.Cloud.job("LeaguePrepareForNextDay", function (request, status) {
	...
	queryLeagues.find({
		success : function (resultsLeagues) {
			...
			var queryFixtures = new Parse.Query("Fixtures");
			queryFixtures.equalTo("League", league);
			queryFixtures.equalTo("GameDay", resultsLeagues[0].get("CurrentDay"));
			queryFixtures.equalTo("Status", 0);
			queryFixtures.select("objectId", "GameDate", "Player0", "Player1", "GameDay", "Status", "Score0", "Score1", "GameFile", "MatchDurationSeconds", "PlayedType");
			queryFixtures.limit(1000);
			...
			queryFixtures.find({
				success : function (resultsFixtures) {
					...
					Parse.Object.saveAll(resultsFixtures, {
						success : function (listSave) {

							status.success("OK!");

						},
						error : function (error) {
							status.error("Fixtures save failed");
						}
					});

				},
				error : function () {
					status.error("Fixtures lookup failed");
				}
			});

		},
		error : function () {
			status.error("League lookup failed");
		}
	});

});

Parse.Cloud.job("GenerateFixturesForServerPlay", function (request, status) {

	var queryUsers = new Parse.Query("UserData");
	queryUsers.equalTo("objectId", "zrcBXRfJgM");
	...
	queryUsers.find({
		...
		success : function (resultsUsers) {
			...
			for (var i = 1; i <= 20; i++) {
				...
				fixture.set("Player0", resultsUsers[0].get("Player"));
				fixture.set("Player1", resultsUsers[0].get("Player"));
				fixture.set("Label", "server");
				fixture.set("Status", 2);
...
Parse.Cloud.job("ResetUserData", function (request, status) {
	...
	queryUsers.find({
		...
		success : function (resultsUsers) {
			...
			Parse.Config.get().then(function (config) {
				var DefaultUserData = config.get("DefaultUserData");

				for (var i = 0; i < resultsUsers.length; i++) {

					resultsUsers[i].set("Currency", UserDefault_Currency);
					resultsUsers[i].set("DailyReward", UserDefault_DailyReward);
					resultsUsers[i].set("Inventory", UserDefault_Inventory);
					resultsUsers[i].set("PlayerCharacterParams", UserDefault_PlayerCharacterParams);
					resultsUsers[i].set("PlayerClubParams", UserDefault_PlayerClubParams);
					resultsUsers[i].set("Statistics", UserDefault_Statistics);
				}

				Parse.Object.saveAll(resultsUsers, {
					useMasterKey : true,
					success : function (list) {

						status.success("Okini-iii");

					},
					error : function (error) {
						// The save failed.  Error is an instance of Parse.Error.
						status.error("Cannot save user data! error.message:" + error.message);
					}
				});

			}, function (error) {
				status.error("Cannot get config! error.message:" + error.message);
			});
...
Parse.Cloud.job("DeleteAllACs", function (request, status) {
	...
	queryUsers.find({
		...
		success : function (resultsUsers) {
			...
			for (var i = 0; i < resultsUsers.length; i++) {
				...
				resultsUsers[i].unset("ACData");
				resultsUsers[i].unset("ACLS");
				resultsUsers[i].unset("ACLSData");
				resultsUsers[i].unset("RefData");
			}
...
Parse.Cloud.job("JobLeaguesAssignUsers", function (request, status) {
	...
	LeaguesAssignUsers().then(function () {
		...
		function (err) {
		status.error("Error: " + JSON.stringify(err));
	});

});

Parse.Cloud.job("JobGenUsers", function (request, status) {

	GenUsers(request.params).then(function () {
		status.success("Ok!");
	},
		function (err) {
		status.error("Error: " + JSON.stringify(err));
...
Parse.Cloud.job("JobLeaguesStart", function (request, status) {
...
Parse.Cloud.job("JobLeaguesNextDay", function (request, status) {
...
Parse.Cloud.job("JobLeaguesPrepareNextDay", function (request, status) {
...
Parse.Cloud.job("JobClearLeagues", function (request, status) {

	var queryL = new Parse.Query("Leagues");
	queryL.limit(1000);

	var queryF = new Parse.Query("Fixtures");
	queryF.limit(1000);

	var queryUD = new Parse.Query("UserData");
	queryUD.exists("League").limit(1000);

	queryL.find().then(function (L) {
		return Parse.Object.destroyAll(L);
	}).then(function () {
		return queryF.find();
	}).then(function (F) {
		return Parse.Object.destroyAll(F);
	}).then(function () {
		return queryF.find();
	}).then(function (F) {
		return Parse.Object.destroyAll(F);
		...
		return Parse.Object.saveAll(UD);
	})
	.then(function () {
		status.success("Ok!");
	},
